...
//  My Contact List
//
//  Created by user246123 on 4/3/24.
//
...
class LocationDemoViewController: UIViewController, CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        if let location = locations.last {
            ...
            if Double(howRecent) < 15.0 {
                ...
                lblLongitude.text = String(format: "%g\u{00B0}", coordinate.longitude)
                lblLatitude.text = String(format: "%g\u{00B0}", coordinate.longitude)
    ...
    func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
        ...
        let alertController = UIAlertController(title: "Error Getting Location: \(errorType)", message: "Erorr Message: \(error.localizedDescription))", preferredStyle: .alert)
